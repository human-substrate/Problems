...
export async function ncvs(): Promise<Result[]> {
  ...
  const recent=stitch(stitch(property(await table(".cache/cv22/cv22t02.csv")),property(await table(".cache/cv23/cv23t02.csv")),"NCVS property 2022/2023"),property(await table(".cache/cv24/cv24t02.csv")),"NCVS property 2022-2023/2024");
  // Disjoint eras have no overlapping observations to compare. Preserve the documented four-year gap.
  invariant(Math.max(...Object.keys(old).map(Number))===2013 && Math.min(...Object.keys(recent).map(Number))===2018,"NCVS disjoint era boundaries");
  return [result("ncvsViolentRate",violent,[0,150],"cv24at01.csv: Year and total violent Rate (column 1); 2006 is publisher -- and omitted."),result("ncvsReportedShare",share,[0,100],"cv24at01.csv: 100 × reported rate (column 7) / total rate (column 1), rounded to 0.1 percentage point; rounded input rates differ from directly published percentages."),result("ncvsPropertyRate",{...old,...recent},[0,500],"cv13f01.csv property column; cv22/cv23/cv24 Table 2 total property rate columns. Every overlapping year must match exactly; disjoint 1993–2013 and 2018–2024 blocks retain a four-year gap.")];
}
export async function corrections(): Promise<Result[]> {
  const jail=stitch(vertical(await table(".cache/ji22st/ji22stt01.csv"),8),vertical(await table(".cache/ji23st/ji23stt01.csv"),7),"jail 2022/2023 editions");
  return [result("jailRate",jail,[0,1000],"ji22stt01.csv column 8 and ji23stt01.csv column 7; 2013–2022 overlap asserted equal."),result("correctionalSupervisionRate",vertical(await table(".cache/cpus23st/cpus23stat01.csv"),1),[0,10000],"cpus23stat01.csv Year and Total under supervision; use this single current-vintage history because the 2022 edition has revised overlap values.")];
}
